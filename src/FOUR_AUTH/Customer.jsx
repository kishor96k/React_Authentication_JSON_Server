...
const Customer = () => {
    ...
    const loadcustomer = () => {
        fetch("http://localhost:3001/customer").then(res => {
            if (!res.ok) {
                return false
            }
            return res.json();
    ...
    const GetUserAccess = () => {
        ...
        fetch("http://localhost:3001/roleaccess?role=" + userrole + "&menu=customer").then(res => {
            if (!res.ok) {
                navigate('/');
    ...
    const handleadd = () => {
        if (haveadd) {
            ...
            alert('You are not having access for add');
        }
    }
    const handleedit = () => {
        if (haveedit) {
            ...
            alert('You are not having access for Edit');
        }
    }
    ...
    const handleremove = () => {
        if (haveremove) {
            ...
            alert('You are not having access for remove');
        }
    }
